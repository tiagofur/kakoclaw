...
```json
{
  "status": "partial",
  "confidence": 0.82,
  "request_help": "The internal Go source files (agents/, bridge/, cron/, session/, persona/, telegram/, config/, runtime/) and bridge/index.ts were not read due to tool iteration limits. A deeper dive into those .go files would raise confidence to 0.95+.",
  "suggestion": "Run a follow-up session focused on reading the internal/ Go source files and .github/workflows/ to complete the module-level analysis."
}
```
