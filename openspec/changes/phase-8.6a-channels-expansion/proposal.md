...
Each adapter: struct embedding `BaseChannel`, implements `Channel` interface. Registered in `initChannels()`. Config struct added to `ChannelsConfig`.
...
|------|------------|------------|
| Config schema migration breaks existing `config.json` | Med | Custom UnmarshalJSON promotes old single-object to 1-element array transparently; unit tests for both forms |
| Session key collision across accounts | Med | Account ID embedded in session key; `isChannelsConfigEmpty` updated |
| WeChat requires server domain verification (callback URL) | High | Document setup requirement; adapter fails gracefully if webhook not reachable |
| Matrix E2E encryption (Olm/Megolm) state management | High | Deliver unencrypted rooms first; E2E deferred to phase 8.6b |
| Nostr private key exposure in config | High | Warn in logs + docs; recommend environment variable for nsec |
| Twilio webhook authentication (signature validation) | Med | Implement `X-Twilio-Signature` validation in webhook handler |
| IRC bot detection / rate limiting | Low | Respect server SLOWMODE; exponential backoff on reconnect |

## Rollback Plan

1. Config schema is additive — old single-object form still parses via `UnmarshalJSON` fallback. Rolling back the binary reverts to single-object behavior with no data loss.
2. New channel adapters are independently gated by `Enabled: false` default — disabling them in config restores prior behavior.
3. `go.mod` additions: `go mod tidy` after rollback removes unused deps.
4. DB: no schema changes in this phase (channels are stateless).

## Dependencies

- `github.com/line/line-bot-sdk-go/v8` — LINE Messaging API
- `github.com/mattermost/mattermost-server/v6/model` — Mattermost Bot API
- `maunium.net/go/mautrix` — Matrix client
- `github.com/nbd-wtf/go-nostr` — Nostr protocol
- `github.com/gempir/go-twitch-irc/v4` — Twitch IRC
- `github.com/thoj/go-ircevent` — IRC
- `github.com/twilio/twilio-go` — Twilio Voice
...
- [ ] All 9 new channel adapters compile and connect in happy-path integration smoke tests
