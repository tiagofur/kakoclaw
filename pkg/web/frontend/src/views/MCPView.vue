<template>
  <div class="flex flex-col h-full bg-makoclaw-bg relative overflow-hidden">
    ...
    <div class="absolute inset-0 pointer-events-none">
      <div class="absolute inset-0 opacity-25 bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-orange-500/30 via-transparent to-transparent"></div>
      <div class="absolute inset-0 opacity-20 bg-[radial-gradient(ellipse_at_bottom_left,_var(--tw-gradient-stops))] from-amber-500/20 via-transparent to-transparent"></div>
    </div>
      ...
      <div class="px-4 sm:px-6 pt-4 sm:pt-5 pb-3">
        <div class="flex items-center gap-3">
          ...
          <div class="flex-1 min-w-0">
            <h1 class="text-xl sm:text-2xl font-bold bg-gradient-to-r from-makoclaw-text via-makoclaw-text to-orange-400 bg-clip-text text-transparent">MCP Servers</h1>
            <p class="text-xs sm:text-sm text-makoclaw-text-secondary mt-0.5">Model Context Protocol connections</p>
          </div>
          ...
          <div class="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-makoclaw-surface/30 backdrop-blur-sm border border-makoclaw-border/30 rounded-xl">
            <span class="relative flex h-2.5 w-2.5">
              <span v-if="connectedCount > 0" class="animate-ping absolute inline-flex h-full w-full rounded-full bg-orange-400 opacity-75"></span>
              <span class="relative inline-flex rounded-full h-2.5 w-2.5" :class="connectedCount > 0 ? 'bg-orange-400' : 'bg-makoclaw-text-secondary/50'"></span>
            </span>
    ...
    <div class="flex-1 overflow-y-auto p-4 sm:p-6 custom-scrollbar">
      ...
      <div v-if="loading" class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div v-for="i in 3" :key="i" class="glass-panel p-5 rounded-xl">
      ...
      <template v-else>
        ...
        <div v-if="servers.length === 0" class="flex flex-col items-center justify-center py-16 text-center">
          <div class="relative">
            <!-- Glow effect -->
            <div class="absolute inset-0 bg-gradient-to-br from-orange-500/30 to-amber-500/30 rounded-3xl blur-2xl opacity-50"></div>
          ...
          <h3 class="text-lg font-bold text-makoclaw-text mt-6">No MCP servers configured</h3>
          <p class="text-sm text-makoclaw-text-secondary/70 mt-2 max-w-xs">Connect external tool servers to extend your agent's capabilities.</p>
          <button
            @click="openAddModal"
          ...
          >
            ...
            <div class="flex items-start justify-between mb-3">
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2">
                  ...
                  <h3 class="font-semibold text-makoclaw-text truncate group-hover:text-orange-400 transition-colors">{{ server.name }}</h3>
                  <!-- Source badge -->
                  <span
                    v-if="server.source"
            ...
            <div v-if="server.connected" class="space-y-2 mb-3">
              <div v-if="server.server_name" class="flex items-center gap-2 text-sm">
                ...
                <span class="text-makoclaw-text">{{ server.server_name }} <span v-if="server.server_version" class="text-makoclaw-text-secondary">v{{ server.server_version }}</span></span>
              </div>
            ...
            <!-- Error Info -->
            <div v-if="server.last_error" class="mb-3 p-3 bg-red-500/5 border border-red-500/20 rounded-xl">
              <p class="text-xs text-red-400 break-all">{{ server.last_error }}</p>
            </div>

            <!-- Test Result -->
            <div v-if="testResults[server.name]" class="mb-3 p-3 rounded-xl border"
        ...
        <!-- Info Box -->
        <div v-if="servers.length > 0" class="mt-6 glass-panel p-4 rounded-xl">
          <div class="flex items-start gap-3">
            ...
            <div>
              <h4 class="text-sm font-semibold text-makoclaw-text mb-1">About MCP</h4>
    ...
    <Transition name="modal">
      <div v-if="showModal" class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-modal p-4" @click.self="showModal = false">
        <div class="bg-makoclaw-surface/95 backdrop-blur-2xl border border-makoclaw-border/50 rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-2xl ring-1 ring-white/10 animate-scaleIn">
            ...
            <div class="flex items-center gap-3">
              <div class="w-9 h-9 rounded-lg bg-gradient-to-br from-orange-500/20 to-amber-500/20 flex items-center justify-center ring-1 ring-white/10">
                <svg class="w-4.5 h-4.5 text-orange-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5.25 14.25h13.5m-13.5 0a3 3 0 01-3-3m3 3a3 3 0 100 6h13.5a3 3 0 100-6m-16.5-3a3 3 0 013-3h13.5a3 3 0 013 3" />
                </svg>
              </div>
              <h3 class="font-bold text-makoclaw-text">{{ editingServer ? 'Edit MCP Server' : 'Add MCP Server' }}</h3>
            </div>
          </div>
          ...
          <div class="p-4 sm:p-6 space-y-4">
            ...
            <div>
              ...
              />
              <p class="text-xs text-makoclaw-text-secondary mt-1.5">Unique identifier for this server (alphanumeric and hyphens)</p>
            </div>
            ...
            <div>
              ...
              />
              <p class="text-xs text-makoclaw-text-secondary mt-1.5">Command to start the MCP server (e.g., npx, node, python)</p>
            </div>
...
/path/to/directory"
              ...
              />
              <p class="text-xs text-makoclaw-text-secondary mt-1.5">One argument per line</p>
            </div>
...
DEBUG=true"
                class="w-full px-3 py-2.5 bg-makoclaw-bg/40 border border-makoclaw-border/50 rounded-xl text-sm outline-none focus:ring-2 focus:ring-orange-500/30 focus:border-orange-500/50 transition-all resize-none font-mono"
              />
              <p class="text-xs text-makoclaw-text-secondary mt-1.5">KEY=value pairs, one per line</p>
            </div>

            <!-- Enabled -->
            <div class="flex items-center gap-3 p-3 bg-makoclaw-bg/30 rounded-xl border border-makoclaw-border/30">
              <input v-model="form.enabled" type="checkbox" id="enabled" class="w-4 h-4 rounded border-makoclaw-border bg-makoclaw-surface text-orange-500 focus:ring-orange-500 transition-all cursor-pointer" />
              <label for="enabled" class="text-sm text-makoclaw-text cursor-pointer">Enable server on save</label>
            </div>
            ...
            <div v-if="editingServer">
              ...
              <div v-if="modalTestResult" class="mt-3 p-3 rounded-xl border"
    ...
    <Transition name="modal">
      <div v-if="showDeleteConfirm" class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-modal p-4" @click.self="showDeleteConfirm = false">
        <div class="bg-makoclaw-surface/95 backdrop-blur-2xl border border-makoclaw-border/50 rounded-2xl max-w-sm w-full shadow-2xl ring-1 ring-white/10 animate-scaleIn">
          ...
          <div class="p-4 border-b border-makoclaw-border/30 bg-gradient-to-r from-red-500/10 to-transparent">
            <div class="flex items-center gap-3">
              ...
              <h3 class="font-bold text-makoclaw-text">Delete Server</h3>
            </div>
          </div>
    ...
    <!-- JSON Edit Modal -->
    <Transition name="modal">
      <div v-if="showJSONModal" class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-modal p-4" @click.self="showJSONModal = false">
          ...
          <div class="p-4 border-b border-makoclaw-border/30 bg-gradient-to-r from-makoclaw-surface/50 to-transparent">
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-3">
                ...
                <h3 class="font-bold text-makoclaw-text">Edit as JSON</h3>
              </div>
              <span v-if="jsonEditingServer" class="text-sm text-makoclaw-text-secondary px-2 py-1 bg-makoclaw-bg/50 rounded-lg">{{ jsonEditingServer.name }}</span>
            </div>
          </div>

          <div class="p-4 sm:p-6">
            <!-- JSON validation error -->
            <div v-if="jsonError" class="mb-4 p-3 bg-red-500/10 border border-red-500/30 rounded-xl">
