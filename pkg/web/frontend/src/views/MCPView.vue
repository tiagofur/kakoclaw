    ...
    <div class="flex-1 overflow-auto p-6 custom-scrollbar">
      ...
      <template v-else>
        ...
        <div v-if="servers.length === 0" class="text-center py-12 text-makoclaw-text-secondary">
          ...
          <p class="text-sm mt-2">Add MCP servers to <code class="px-1.5 py-0.5 bg-makoclaw-surface rounded text-xs">config.json</code> under <code class="px-1.5 py-0.5 bg-makoclaw-surface rounded text-xs">tools.mcp.servers</code></p>
          <div class="mt-4 max-w-lg mx-auto text-left">
            <pre class="bg-makoclaw-surface border border-makoclaw-border rounded-lg p-4 text-xs overflow-x-auto"><code>{
  "tools": {
    "mcp": {
      "servers": {
        "my-server": {
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2">
                  <span
                    ...
                    :class="server.connected ? 'bg-blue-400' : 'bg-red-400'"
                  ></span>
                  <h3 class="font-semibold truncate">{{ server.name }}</h3>
                </div>
                <p class="text-xs text-makoclaw-text-secondary mt-1 font-mono truncate" :title="server.command">{{ server.command }}</p>
              </div>
              <span
