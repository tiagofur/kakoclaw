  ...
  <div class="h-full flex flex-col">
    ...
    <div class="flex-none p-4 border-b border-kakoclaw-border bg-kakoclaw-surface sticky top-0 z-10">
      <div class="flex justify-between items-center mb-3">
        ...
        <div class="flex items-center gap-2">
          <div class="relative" ref="exportDropdownRef">
            ...
            <div v-if="showExportMenu" class="absolute right-0 top-full mt-1 w-48 bg-kakoclaw-surface border border-kakoclaw-border rounded-lg shadow-lg p-1 z-50">
              <button @click="handleExportAll" class="w-full text-left px-3 py-2 hover:bg-kakoclaw-bg rounded text-sm transition-colors">Export All Chats</button>
              <button v-if="activeSessionId" @click="handleExportSession" class="w-full text-left px-3 py-2 hover:bg-kakoclaw-bg rounded text-sm transition-colors">Export Current Session</button>
            </div>
          </div>
          <button @click="triggerImport" class="p-2 hover:bg-kakoclaw-bg rounded-lg text-kakoclaw-text-secondary transition-colors" title="Import conversations (ChatGPT, Claude, KakoClaw)">
          ...
          <button @click="loadSessions" class="p-2 hover:bg-kakoclaw-bg rounded-lg text-kakoclaw-text-secondary transition-colors" title="Refresh">
      ...
      <div class="relative mb-3">
        <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-kakoclaw-text-secondary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
        <input
          v-model="searchQuery"
          @input="onSearchInput"
          type="text"
          placeholder="Search message content..."
          class="w-full pl-10 pr-8 py-2 bg-kakoclaw-bg border border-kakoclaw-border rounded-lg text-sm outline-none focus:border-kakoclaw-accent transition-colors"
        />
        <button
          v-if="searchQuery"
          @click="clearSearch"
          class="absolute right-2 top-1/2 -translate-y-1/2 p-1 hover:bg-kakoclaw-surface rounded text-kakoclaw-text-secondary"
        >
      ...
      <div class="flex items-center gap-3">
        <select v-model="filterType" class="bg-kakoclaw-bg border border-kakoclaw-border rounded-lg px-3 py-1.5 text-sm outline-none focus:border-kakoclaw-accent">
          <option value="all">All Sessions</option>
          <option value="chat">Chats Only</option>
          <option value="task">Tasks Only</option>
        </select>
        <select v-model="filterDate" class="bg-kakoclaw-bg border border-kakoclaw-border rounded-lg px-3 py-1.5 text-sm outline-none focus:border-kakoclaw-accent">
          <option value="all">All Time</option>
          <option value="today">Today</option>
          <option value="7d">Last 7 Days</option>
          <option value="30d">Last 30 Days</option>
        </select>
        <label class="flex items-center gap-1.5 text-sm text-kakoclaw-text-secondary cursor-pointer">
          <input type="checkbox" v-model="showArchived" @change="loadSessions" class="rounded border-kakoclaw-border text-kakoclaw-accent focus:ring-kakoclaw-accent" />
          Archived
        </label>
        <span class="text-xs text-kakoclaw-text-secondary ml-auto">
    ...
    <div class="flex-1 flex overflow-hidden">
      ...
      <div class="w-1/3 border-r border-kakoclaw-border overflow-y-auto p-2 space-y-1 bg-kakoclaw-surface/50">
        ...
        <template v-else-if="isSearchMode">
          <div v-if="filteredSearchResults.length === 0" class="text-center py-4 text-kakoclaw-text-secondary">
          ...
          <div
            ...
            class="p-3 rounded-lg cursor-pointer transition-all border border-transparent hover:border-kakoclaw-border hover:bg-kakoclaw-bg/50"
            :class="selectedSearchResult === idx ? 'bg-kakoclaw-bg border-kakoclaw-accent/30 shadow-sm' : ''"
          >
            <div class="flex items-center gap-2 mb-1">
              ...
              <span class="text-[10px] px-1.5 py-0.5 rounded"
                :class="isTaskSession(result) ? 'bg-amber-500/20 text-amber-400' : 'bg-emerald-500/20 text-emerald-400'">
                {{ isTaskSession(result) ? 'Task' : 'Chat' }}
              </span>
            </div>
            <div class="text-sm truncate text-kakoclaw-text" v-html="highlightMatch(result.content, searchQuery)"></div>
            <div class="text-[10px] text-kakoclaw-text-secondary mt-1">{{ formatTime(result.created_at) }}</div>
          </div>
        </template>

        <!-- Normal sessions mode -->
        <template v-else>
          <div v-if="filteredSessions.length === 0" class="text-center py-4 text-kakoclaw-text-secondary">No matching history found.</div>

          <div
            v-for="session in filteredSessions"
            :key="session.session_id"
            @click="selectSession(session)"
            class="p-3 rounded-lg cursor-pointer transition-all border border-transparent hover:border-kakoclaw-border group"
            :class="selectedSession?.session_id === session.session_id ? 'bg-kakoclaw-bg border-kakoclaw-accent/30 shadow-sm' : 'hover:bg-kakoclaw-bg/50'"
          >
            <!-- Inline rename -->
            ...
            <template v-else>
              <div class="flex items-center gap-2 mb-1">
                ...
                <div class="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity" @click.stop>
                  <button @click="startRenameSession(session)" class="p-1 hover:bg-kakoclaw-border rounded text-kakoclaw-text-secondary hover:text-kakoclaw-accent transition-colors" title="Rename">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                  </button>
                  <button @click="archiveSessionById(session.session_id)" class="p-1 hover:bg-kakoclaw-border rounded text-kakoclaw-text-secondary hover:text-amber-400 transition-colors" :title="showArchived ? 'Unarchive' : 'Archive'">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4" /></svg>
                  </button>
                  <button @click="deleteSessionById(session.session_id)" class="p-1 hover:bg-kakoclaw-border rounded text-kakoclaw-text-secondary hover:text-kakoclaw-error transition-colors" title="Delete">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                  </button>
                </div>
              </div>
              <div class="text-xs text-kakoclaw-text-secondary flex justify-between items-center mt-1.5 pl-6">
                <span>{{ formatTime(session.updated_at) }}</span>
                <div class="flex items-center gap-2">
                  <span v-if="session.message_count" class="text-[10px] text-kakoclaw-text-secondary">
      ...
      <!-- Message View -->
      <div class="flex-1 overflow-y-auto p-4 bg-kakoclaw-bg/30 relative">
        <div v-if="!selectedSession && !selectedSearchResultSession" class="h-full flex flex-col items-center justify-center text-kakoclaw-text-secondary">
        ...
        <div v-else class="space-y-4 max-w-3xl mx-auto pb-20">
          <div class="text-center mb-6 sticky top-0 z-10 bg-kakoclaw-bg/95 backdrop-blur py-2 border-b border-kakoclaw-border/50 flex justify-between items-center px-4 rounded-b-xl shadow-sm">
            <div class="flex items-center gap-2 text-xs text-kakoclaw-text-secondary">
            ...
            <button
              @click="continueChat"
              class="flex items-center gap-2 px-3 py-1.5 bg-kakoclaw-accent text-white text-xs rounded-lg hover:bg-kakoclaw-accent-hover transition-colors shadow-sm"
            >
          ...
          <div v-if="loadingMessages" class="text-center py-8">
            <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-kakoclaw-accent mx-auto"></div>
          </div>
          ...
          >
            <div class="text-xs text-kakoclaw-text-secondary px-1 opacity-70 flex items-center gap-2">
              <span>{{ msg.role }}</span>
              <button
                v-if="msg.role === 'assistant'"
                @click="copyMessageContent(msg.content)"
                class="opacity-0 group-hover:opacity-100 transition-opacity p-0.5 hover:bg-kakoclaw-surface rounded text-kakoclaw-text-secondary hover:text-kakoclaw-accent"
                title="Copy response"
              >
            ...
            <div
              ...
              :class="msg.role === 'user'
                ? 'bg-kakoclaw-accent text-white rounded-tr-none whitespace-pre-wrap'
                : 'bg-white dark:bg-gray-800 border border-kakoclaw-border rounded-tl-none'"
            >
...
const highlightMatch = (text, query) => {
  ...
  return truncated.replace(
    ...
    '<mark class="bg-kakoclaw-accent/30 text-kakoclaw-text rounded px-0.5">$1</mark>'
  )
}
