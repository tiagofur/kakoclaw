<template>
  <div class="h-full flex flex-col bg-picoclaw-bg">
    ...
    <div class="flex-none p-4 border-b border-picoclaw-border bg-picoclaw-surface flex items-center justify-between">
      <div>
        <h2 class="text-xl font-bold bg-gradient-to-r from-picoclaw-accent to-purple-500 bg-clip-text text-transparent">
          {{ editing ? (editingWorkflow.id ? 'Edit Workflow' : 'New Workflow') : 'Workflows' }}
        </h2>
        <p class="text-sm text-picoclaw-text-secondary mt-1">
          {{ editing ? 'Visual pipeline builder' : 'Create and manage automation pipelines' }}
        </p>
      </div>
      <div class="flex items-center gap-2">
        <button v-if="editing" @click="cancelEdit"
          class="flex items-center gap-2 px-4 py-2 text-picoclaw-text-secondary hover:text-picoclaw-text border border-picoclaw-border rounded-lg transition-colors text-sm">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
          Cancel
        </button>
        <button v-if="editing" @click="saveWorkflow" :disabled="saving"
          class="flex items-center gap-2 px-4 py-2 bg-picoclaw-accent text-white rounded-lg hover:bg-picoclaw-accent/90 transition-colors text-sm disabled:opacity-50">
    ...
    <div class="flex-1 overflow-auto p-6 custom-scrollbar">
      ...
      <template v-else-if="!editing">
        <div v-if="workflows.length === 0" class="text-center py-12 text-picoclaw-text-secondary">
        ...
        <div class="space-y-3">
          <div v-for="wf in workflows" :key="wf.id"
            class="bg-picoclaw-surface border border-picoclaw-border rounded-xl p-5 hover:border-picoclaw-accent/30 transition-colors">
            <div class="flex items-start justify-between">
              <div class="flex-1 min-w-0">
                ...
                <p v-if="wf.description" class="text-sm text-picoclaw-text-secondary mt-1">{{ wf.description }}</p>
              </div>
            </div>
            ...
            <div class="flex items-center gap-2 mt-3">
              <button @click="startEdit(wf)"
                class="px-3 py-1.5 text-xs bg-picoclaw-accent/10 text-picoclaw-accent rounded-lg hover:bg-picoclaw-accent/20 transition-colors">
                Edit
              </button>
            ...
            <!-- Inline Run Results -->
            <div v-if="wf._lastResults" class="mt-4 border-t border-picoclaw-border pt-3">
              <h4 class="text-xs font-semibold text-picoclaw-text-secondary mb-2">Last Run Results</h4>
              <div class="space-y-2">
                <div v-for="(res, i) in wf._lastResults" :key="i"
                  class="text-xs p-2 rounded-lg bg-picoclaw-bg border border-picoclaw-border">
                  <div class="flex items-center gap-2 mb-1">
                    ...
                    <span v-if="res.duration_ms" class="text-picoclaw-text-secondary">{{ res.duration_ms }}ms</span>
                  </div>
                  <pre v-if="res.output" class="whitespace-pre-wrap text-picoclaw-text-secondary max-h-24 overflow-auto">{{ res.output }}</pre>
            ...
            <!-- Inline Runs History -->
            <div v-if="wf._runs" class="mt-4 border-t border-picoclaw-border pt-3">
              <div class="flex items-center justify-between mb-2">
                <h4 class="text-xs font-semibold text-picoclaw-text-secondary">Recent Runs</h4>
                <button @click="wf._runs = null" class="text-xs text-picoclaw-text-secondary hover:text-picoclaw-text">Hide</button>
              </div>
              <div v-if="wf._runs.length === 0" class="text-xs text-picoclaw-text-secondary">No runs yet</div>
              <div class="space-y-1">
                <div v-for="run in wf._runs" :key="run.id"
                  class="text-xs flex items-center gap-3 p-2 rounded bg-picoclaw-bg">
                  ...
                  <span class="text-picoclaw-text-secondary">{{ formatDate(run.started_at) }}</span>
                  <span v-if="run.finished_at" class="text-picoclaw-text-secondary">
                    ({{ runDuration(run) }})
                  </span>
                </div>
              </div>
      ...
      <template v-else>
        <!-- Workflow Meta -->
        <div class="bg-picoclaw-surface border border-picoclaw-border rounded-xl p-5 mb-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="block text-xs font-semibold text-picoclaw-text-secondary mb-1">Name</label>
              <input v-model="editingWorkflow.name" type="text" placeholder="My Workflow"
                class="w-full px-3 py-2 bg-picoclaw-bg border border-picoclaw-border rounded-lg text-sm focus:outline-none focus:border-picoclaw-accent" />
            </div>
            <div>
              <label class="block text-xs font-semibold text-picoclaw-text-secondary mb-1">Description</label>
        ...
        <!-- Pipeline Steps -->
        <div class="mb-4">
          <h3 class="text-sm font-semibold text-picoclaw-text-secondary mb-3">Pipeline Steps</h3>

          <VueDraggable v-model="editingWorkflow.steps" :animation="200" handle=".drag-handle"
            class="space-y-3">
            <template #item="{ element: step, index }">
              <div :key="step.id" class="bg-picoclaw-surface border border-picoclaw-border rounded-xl overflow-hidden"
                :class="expandedStep === step.id ? 'ring-1 ring-picoclaw-accent/50' : ''">
                <!-- Step Header -->
                <div class="flex items-center gap-3 px-4 py-3 cursor-pointer" @click="toggleStep(step.id)">
                  <div class="drag-handle cursor-grab text-picoclaw-text-secondary hover:text-picoclaw-text">
                  ...
                  <button @click.stop="removeStep(index, step.id)"
                    class="p-1 text-picoclaw-text-secondary hover:text-red-400 transition-colors">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                    </svg>
                  </button>
                  <svg class="w-4 h-4 text-picoclaw-text-secondary transition-transform" :class="expandedStep === step.id ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                  </svg>
                </div>

                <!-- Step Config (expanded) -->
                <div v-if="expandedStep === step.id" class="px-4 pb-4 border-t border-picoclaw-border pt-3 space-y-3">
                  <div>
                    <label class="block text-xs font-semibold text-picoclaw-text-secondary mb-1">Label</label>
                    <input v-model="step.label" type="text" placeholder="Step label"
                      class="w-full px-3 py-2 bg-picoclaw-bg border border-picoclaw-border rounded-lg text-sm focus:outline-none focus:border-picoclaw-accent" />
                  </div>

                  <div>
                    <label class="block text-xs font-semibold text-picoclaw-text-secondary mb-1">On Error</label>
                  ...
                  <template v-if="step.type === 'prompt'">
                    <div>
                      <label class="block text-xs font-semibold text-picoclaw-text-secondary mb-1">
                        Message
                      ...
                      <textarea v-model="step._config.message" rows="4" placeholder="Enter prompt message..."
                        class="w-full px-3 py-2 bg-picoclaw-bg border border-picoclaw-border rounded-lg text-sm focus:outline-none focus:border-picoclaw-accent font-mono resize-y"></textarea>
                    </div>
                    <div>
                      <label class="block text-xs font-semibold text-picoclaw-text-secondary mb-1">Model Override (optional)</label>
                  ...
                  <template v-if="step.type === 'tool'">
                    <div>
                      <label class="block text-xs font-semibold text-picoclaw-text-secondary mb-1">Tool Name</label>
                    ...
                    <div>
                      ...
                      <textarea v-model="step._config._argsJson" rows="4" placeholder='{"key": "value"}'
                        class="w-full px-3 py-2 bg-picoclaw-bg border border-picoclaw-border rounded-lg text-sm focus:outline-none focus:border-picoclaw-accent font-mono resize-y"
                  ...
                  <template v-if="step.type === 'condition'">
                    <div>
                      ...
                      <input v-model="step._config.reference" type="text" placeholder="{{step.1.output}}"
                        class="w-full px-3 py-2 bg-picoclaw-bg border border-picoclaw-border rounded-lg text-sm focus:outline-none focus:border-picoclaw-accent font-mono" />
                    </div>
                    <div>
                      <label class="block text-xs font-semibold text-picoclaw-text-secondary mb-1">Operator</label>
                    ...
                    <div v-if="step._config.operator !== 'not_empty'">
                      <label class="block text-xs font-semibold text-picoclaw-text-secondary mb-1">Value</label>
        ...
        <div class="bg-picoclaw-surface border border-picoclaw-border rounded-xl p-5">
          <div class="flex items-center justify-between mb-3">
            ...
            <span v-else class="text-xs text-picoclaw-text-secondary">Save the workflow first to test it</span>
          </div>

          <div v-if="testResults" class="space-y-2">
            <div v-for="(res, i) in testResults" :key="i"
              class="text-xs p-3 rounded-lg bg-picoclaw-bg border border-picoclaw-border">
              <div class="flex items-center gap-2 mb-1">
                <span class="font-mono font-semibold">{{ res.label || res.step_id }}</span>
                <span class="px-1.5 py-0.5 rounded text-[10px]"
                  :class="res.error ? 'bg-red-500/10 text-red-400' : res.skipped ? 'bg-gray-500/10 text-gray-400' : 'bg-emerald-500/10 text-emerald-400'">
              ...
              <pre v-if="res.output" class="whitespace-pre-wrap text-picoclaw-text-secondary max-h-32 overflow-auto">{{ res.output }}</pre>
              <pre v-if="res.error" class="whitespace-pre-wrap text-red-400 max-h-32 overflow-auto">{{ res.error }}</pre>
            </div>
          </div>
          <div v-else class="text-xs text-picoclaw-text-secondary">
