<template>
  <div class="h-full flex flex-col bg-picoclaw-bg">
    ...
    <div class="flex-none p-4 border-b border-picoclaw-border bg-picoclaw-surface">
      <h2 class="text-xl font-bold bg-gradient-to-r from-picoclaw-accent to-purple-500 bg-clip-text text-transparent">Dashboard</h2>
      <p class="text-sm text-picoclaw-text-secondary mt-1">Overview of your PicoClaw workspace</p>
    </div>
    ...
    <div class="flex-1 overflow-auto p-6 space-y-6 custom-scrollbar">
      ...
      <template v-else>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div class="bg-picoclaw-surface border border-picoclaw-border rounded-xl p-4">
            <div class="text-sm text-picoclaw-text-secondary">Total Tasks</div>
            <div class="text-2xl font-bold mt-1">{{ stats.totalTasks }}</div>
          </div>
          <div class="bg-picoclaw-surface border border-picoclaw-border rounded-xl p-4">
            <div class="text-sm text-picoclaw-text-secondary">In Progress</div>
            <div class="text-2xl font-bold mt-1 text-blue-400">{{ stats.inProgress }}</div>
          </div>
          <div class="bg-picoclaw-surface border border-picoclaw-border rounded-xl p-4">
            <div class="text-sm text-picoclaw-text-secondary">Chat Sessions</div>
            <div class="text-2xl font-bold mt-1 text-emerald-400">{{ stats.chatSessions }}</div>
          </div>
          <div class="bg-picoclaw-surface border border-picoclaw-border rounded-xl p-4">
            <div class="text-sm text-picoclaw-text-secondary">Total Messages</div>
        ...
        <div class="bg-picoclaw-surface border border-picoclaw-border rounded-xl p-5">
          ...
          <div class="grid grid-cols-2 sm:grid-cols-5 gap-3">
            <div v-for="s in statusBreakdown" :key="s.status" class="text-center p-3 rounded-lg bg-picoclaw-bg">
              <div class="text-lg font-bold" :class="s.color">{{ s.count }}</div>
              <div class="text-xs text-picoclaw-text-secondary mt-1 capitalize">{{ s.label }}</div>
            </div>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div class="bg-picoclaw-surface border border-picoclaw-border rounded-xl p-5">
            <div class="flex items-center justify-between mb-4">
              ...
              <router-link to="/tasks" class="text-sm text-picoclaw-accent hover:underline">View all</router-link>
            </div>
            <div v-if="recentTasks.length === 0" class="text-sm text-picoclaw-text-secondary py-4 text-center">No tasks yet</div>
            <div v-else class="space-y-2">
              <div
                v-for="task in recentTasks"
                :key="task.id"
                class="flex items-center gap-3 p-3 rounded-lg bg-picoclaw-bg hover:bg-picoclaw-border/50 transition-colors"
              >
                <span
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div class="text-xs text-picoclaw-text-secondary capitalize">{{ task.status.replace('_', ' ') }}</div>
                </div>
                <div class="text-xs text-picoclaw-text-secondary flex-shrink-0">
          ...
          <div class="bg-picoclaw-surface border border-picoclaw-border rounded-xl p-5">
            <div class="flex items-center justify-between mb-4">
              ...
              <router-link to="/history" class="text-sm text-picoclaw-accent hover:underline">View all</router-link>
            </div>
            <div v-if="recentSessions.length === 0" class="text-sm text-picoclaw-text-secondary py-4 text-center">No conversations yet</div>
            <div v-else class="space-y-2">
              <div
              ...
              >
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div class="text-xs text-picoclaw-text-secondary">{{ session.message_count }} messages</div>
                </div>
                <div class="text-xs text-picoclaw-text-secondary flex-shrink-0">
        ...
        <div class="bg-picoclaw-surface border border-picoclaw-border rounded-xl p-5">
          ...
          <div class="flex flex-wrap gap-3">
            ...
            <router-link
              to="/tasks"
              class="flex items-center gap-2 px-4 py-2 bg-picoclaw-bg border border-picoclaw-border text-picoclaw-text rounded-lg hover:bg-picoclaw-border/50 transition-colors text-sm"
            >
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" /></svg>
              New Task
            </router-link>
