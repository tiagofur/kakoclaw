      ...
      <div class="border-t border-makoclaw-border/30 bg-makoclaw-surface/50 backdrop-blur-2xl p-3 sm:p-4 z-20 relative ring-1 ring-white/[0.05]">
        ...
        <form @submit.prevent="sendMessage" class="flex flex-col gap-1.5 sm:gap-2 max-w-4xl mx-auto w-full">
          ...
          <div v-if="attachments.length > 0" class="flex flex-wrap gap-2 px-1">
            ...
            <div v-if="uploadingFile" class="flex items-center gap-1.5 px-2.5 py-1.5 text-xs text-makoclaw-text-secondary">
              <div class="w-3 h-3 border-2 border-makoclaw-accent border-t-transparent rounded-full animate-spin"></div>
              Uploading...
            </div>
          </div>
