<template>
  <div class="h-full flex flex-col bg-makoclaw-bg relative overflow-hidden">
    ...
    <div class="absolute inset-0 pointer-events-none">
      <div class="absolute inset-0 opacity-25 bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-lime-500/30 via-transparent to-transparent" />
      <div class="absolute inset-0 opacity-20 bg-[radial-gradient(ellipse_at_bottom_left,_var(--tw-gradient-stops))] from-green-500/20 via-transparent to-transparent" />
    </div>
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-xs sm:text-sm text-makoclaw-text-secondary mt-0.5">
              Manage your AI specialists and track their performance
            </p>
          </div>
    ...
    <div class="flex-1 overflow-y-auto p-4 md:p-6 custom-scrollbar relative">
        ...
        <div class="glass-panel rounded-2xl p-6 ring-1 ring-white/10">
          <div class="flex items-center justify-between mb-4">
            <div class="flex items-center gap-3">
              ...
              <div>
                <div class="skeleton h-4 w-28 mb-2 rounded" />
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
            <div
              v-for="i in 4"
              :key="'orch-'+i"
              class="p-3 bg-makoclaw-bg/40 rounded-xl"
            >
              <div class="skeleton h-2.5 w-16 mb-2 rounded" />
        ...
        <div class="space-y-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div
              v-for="i in 3"
              :key="'spec-'+i"
              class="glass-panel rounded-xl p-5 ring-1 ring-white/10"
            >
              ...
              <div class="skeleton h-3 w-full mb-1 rounded" />
              <div class="skeleton h-3 w-2/3 mb-4 rounded" />
              <div class="flex gap-1 mb-4">
                <div class="skeleton h-4 w-14 rounded-full" />
      ...
      >
        <!-- Orchestrator Status -->
        <div
          class="glass-panel rounded-2xl p-5 sm:p-6 ring-1"
          :class="orchestrator.enabled ? 'ring-lime-500/30 bg-lime-500/5' : 'ring-white/10'"
        >
          <div class="flex items-center justify-between mb-4">
            <div class="flex items-center gap-3">
              <div :class="`w-11 h-11 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center ring-1 ring-white/10 shadow-lg ${orchestrator.enabled ? 'bg-gradient-to-br from-lime-500/20 to-green-500/20 shadow-lime-500/10' : 'bg-makoclaw-bg/60'}`">
                <svg
                  ...
                  :class="orchestrator.enabled ? 'text-lime-400' : 'text-makoclaw-text-secondary'"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
              ...
              <div>
                ...
                <p class="text-sm text-makoclaw-text-secondary">
                  {{ orchestrator.enabled ? 'Active - Delegating tasks to specialists' : 'Disabled' }}
                </p>
              </div>
            </div>
          ...
          <div
            v-if="orchestrator.enabled"
            class="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mt-4"
          >
            <div class="p-3 sm:p-4 bg-makoclaw-bg/40 rounded-xl border border-makoclaw-border/30 backdrop-blur-sm">
              <p class="text-[10px] sm:text-xs text-makoclaw-text-secondary uppercase tracking-wider mb-1 font-medium">
                Provider
              </p>
              <p class="font-bold text-makoclaw-text text-sm sm:text-base">
            ...
            <div class="p-3 sm:p-4 bg-makoclaw-bg/40 rounded-xl border border-makoclaw-border/30 backdrop-blur-sm">
              ...
              <p class="font-bold text-makoclaw-text text-xs sm:text-sm truncate">
                {{ orchestrator.model }}
              </p>
            </div>
            <div class="p-3 sm:p-4 bg-makoclaw-bg/40 rounded-xl border border-makoclaw-border/30 backdrop-blur-sm">
            ...
            <div class="p-3 sm:p-4 bg-makoclaw-bg/40 rounded-xl border border-makoclaw-border/30 backdrop-blur-sm">
              ...
              <p class="font-bold text-makoclaw-text text-sm sm:text-base">
                {{ orchestrator.max_delegation_retries }}
              </p>
            </div>
          </div>
          ...
          <div
            v-else
            class="mt-4 p-5 bg-makoclaw-bg/40 rounded-xl border border-dashed border-makoclaw-border/30 text-center"
          >
            <svg
              class="w-10 h-10 mx-auto text-makoclaw-text-secondary/30 mb-3"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
            ...
            <p class="text-sm text-makoclaw-text-secondary mb-4">
              Orchestrator is not enabled. Configure it in Settings > Agents.
            </p>
            <router-link
        ...
        <div class="space-y-4">
          ...
          <!-- Empty State -->
          <div
            v-if="agentsStore.specialists.length === 0"
            class="flex flex-col items-center justify-center py-12 text-center"
          >
            <div class="relative">
              <div class="absolute inset-0 bg-gradient-to-br from-lime-500/30 to-green-500/30 rounded-3xl blur-2xl opacity-50" />
            ...
            <p class="text-sm text-makoclaw-text-secondary/70 mt-2 max-w-md">
              Create your first AI specialist to start using the multi-agent system. Each specialist can be configured with specific tools and capabilities.
            </p>
            <router-link
              to="/settings?tab=agents"
          ...
          >
            ...
            >
              <div class="flex items-start justify-between mb-3">
                <div class="flex items-center gap-3">
                  <div :class="`w-10 h-10 rounded-lg ${agentsStore.getSpecialistBgColor(specialist.name)} flex items-center justify-center ring-1 ring-white/10`">
                    <svg
                      class="w-5 h-5"
                      :class="agentsStore.getSpecialistColor(specialist.name)"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                      v-html="agentsStore.getSpecialistIcon(specialist.name)"
                    />
                  </div>
                  <div>
                    <h4 class="font-bold text-makoclaw-text capitalize">
                      {{ specialist.name }}
                    </h4>
              ...
              <p class="text-sm text-makoclaw-text-secondary mb-4 line-clamp-2">
              ...
              <div class="flex flex-wrap gap-1.5 mb-4">
                ...
                <span
                  v-if="(specialist.keywords || []).length > 3"
                  class="px-2 py-0.5 text-[10px] font-medium text-makoclaw-text-secondary/60"
                >
              ...
              <div class="pt-3 border-t border-makoclaw-border/30 flex items-center justify-between text-xs">
                <div class="flex items-center gap-4 text-makoclaw-text-secondary">
                  <span class="flex items-center gap-1.5">
                    <svg
                      class="w-3.5 h-3.5 text-lime-400/60"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                  ...
                  <span class="flex items-center gap-1.5">
                    <svg
                      class="w-3.5 h-3.5 text-green-400/60"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
        ...
        >
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div
              class="glass-panel rounded-xl p-5 ring-1 ring-white/10 animate-fadeUp"
            ...
            >
              <div class="flex items-center gap-3 mb-3">
                ...
                <div>
                  <p class="text-xs text-makoclaw-text-secondary uppercase tracking-wider font-medium">
                    Total Cost
                  </p>
          ...
          <div class="glass-panel rounded-xl p-5 ring-1 ring-white/10">
            ...
            <div
              v-if="Object.keys(metrics.by_specialist || {}).length === 0"
              class="text-sm text-makoclaw-text-secondary/60 text-center p-4 bg-makoclaw-bg/30 rounded-lg border border-dashed border-makoclaw-border/30"
            >
              ...
              >
                <div class="flex items-center gap-3">
                  <div :class="`w-8 h-8 rounded-lg ${agentsStore.getSpecialistBgColor(name)} flex items-center justify-center ring-1 ring-white/10`">
                    <svg
                      class="w-4 h-4"
                      :class="agentsStore.getSpecialistColor(name)"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                      v-html="agentsStore.getSpecialistIcon(name)"
                    />
                  </div>
