  ...
  <div class="space-y-5 max-w-2xl mx-auto animate-fade-in-up">
    ...
    <div class="glass-panel rounded-2xl p-5 md:p-6 border border-makoclaw-border/50 relative overflow-hidden group">
      ...
      <div class="relative z-10">
        ...
        <div v-else class="space-y-5">
          <div v-if="errorMessage" class="p-3 bg-red-500/10 border border-red-500/20 rounded-xl text-red-400 text-xs font-medium">
            {{ errorMessage }}
          </div>
          <div v-if="successMessage" class="p-3 bg-makoclaw-success/10 border border-makoclaw-success/20 rounded-xl text-makoclaw-success text-xs font-medium">
            {{ successMessage }}
          </div>

          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="space-y-2">
              <label class="text-[10px] font-medium uppercase tracking-wide text-makoclaw-text-secondary/60 ml-1">Username</label>
              <input v-model="profile.username" type="text" class="w-full bg-makoclaw-bg/40 border border-makoclaw-border/50 rounded-xl px-4 py-2.5 text-sm font-medium text-makoclaw-text focus:border-makoclaw-accent outline-none transition-all" />
              <p class="text-[9px] font-medium text-makoclaw-text-secondary/40 italic ml-1">* Identity change requires re-authentication</p>
            </div>

            <div class="space-y-2">
              <label class="text-[10px] font-medium uppercase tracking-wide text-makoclaw-text-secondary/60 ml-1">Communication Link</label>
              <input v-model="profile.email" type="email" class="w-full bg-makoclaw-bg/40 border border-makoclaw-border/50 rounded-xl px-4 py-2.5 text-sm font-medium text-makoclaw-text focus:border-makoclaw-accent outline-none transition-all" placeholder="commander@mako.net" />
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 gap-4 p-3 rounded-xl bg-makoclaw-bg/20 border border-makoclaw-border/30">
            ...
            <div>
              ...
              <span class="text-[11px] font-medium text-makoclaw-text">{{ formatDate(profile.created_at) }}</span>
            </div>
          </div>
    ...
    <div class="glass-panel rounded-2xl p-4 bg-blue-500/5 border border-blue-500/10">
       <div class="flex items-start gap-3">
          ...
          <div>
            <h4 class="text-[11px] font-medium tracking-wide text-makoclaw-text">Protocol Sync</h4>
    ...
    <Teleport to="body">
      ...
      <div v-if="showChangePassword" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-xl p-4" @click.self="showChangePassword = false">
        <div class="glass-panel rounded-2xl p-5 max-w-md w-full shadow-xl border border-makoclaw-border/50 relative overflow-hidden animate-zoom" @click.stop>
          <div class="absolute -top-12 -left-12 w-48 h-48 bg-blue-500/10 blur-[60px] rounded-full pointer-events-none"></div>

          <h3 class="text-xl font-semibold text-makoclaw-text mb-5">Update Shield Key</h3>

          <div v-if="passwordError" class="p-3 bg-red-500/10 border border-red-500/20 rounded-xl text-red-400 text-[11px] font-medium mb-4">
            {{ passwordError }}
          </div>

          <div class="space-y-6">
            <div class="space-y-2">
              <label class="text-[10px] font-medium uppercase tracking-wide text-makoclaw-text-secondary/60 ml-1">Current Key</label>
            ...
            <div class="space-y-2">
              <label class="text-[10px] font-medium uppercase tracking-wide text-makoclaw-text-secondary/60 ml-1">New Neural Hash</label>
            ...
            <div class="space-y-2">
              <label class="text-[10px] font-medium uppercase tracking-wide text-makoclaw-text-secondary/60 ml-1">Verify Hash</label>
          ...
          <div class="flex gap-4 mt-10 pt-8 border-t border-makoclaw-border/30">
            <button @click="showChangePassword = false" class="flex-1 px-4 py-2.5 text-xs font-medium text-makoclaw-text-secondary hover:text-makoclaw-text transition-colors">Abort</button>
            <button @click="changePassword" :disabled="changingPassword" class="flex-1 px-4 py-2.5 bg-makoclaw-accent text-white rounded-xl font-semibold shadow-lg shadow-makoclaw-accent/20 active:scale-95 disabled:opacity-30 flex items-center justify-center">
