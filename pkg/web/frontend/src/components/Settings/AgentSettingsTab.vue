<template>
  <div class="space-y-5 max-w-4xl mx-auto animate-fade-in-up">
    ...
    <div class="glass-panel rounded-2xl p-5 md:p-6 border border-makoclaw-border/50 relative overflow-hidden group">
      ...
      <div class="relative z-10">
        ...
        <Transition name="fade-slide">
          <div v-if="orchestratorConfig.enabled" class="space-y-5 pt-5 border-t border-makoclaw-border/30">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div class="space-y-2">
                <label class="text-[10px] font-medium uppercase tracking-wide text-makoclaw-text-secondary/60 ml-1">Command Provider</label>
                <select v-model="orchestratorConfig.provider" class="w-full bg-makoclaw-bg/40 border border-makoclaw-border/50 rounded-xl px-4 py-2.5 text-sm font-medium text-makoclaw-text focus:border-makoclaw-accent outline-none transition-all cursor-pointer">
                  <option v-for="p in configuredProviders" :key="p.name" :value="p.name">{{ p.name.toUpperCase() }}</option>
                </select>
              </div>
              <div class="space-y-2">
                <label class="text-[10px] font-medium uppercase tracking-wide text-makoclaw-text-secondary/60 ml-1">Primary Model Core</label>
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <div class="space-y-2">
                <label class="text-[10px] font-medium uppercase tracking-wide text-makoclaw-text-secondary/60 ml-1">Cognitive Limit</label>
                <input v-model.number="orchestratorConfig.max_tokens" type="number" class="w-full bg-makoclaw-bg/20 border border-makoclaw-border/30 rounded-xl px-4 py-2.5 text-sm font-medium text-makoclaw-text focus:border-makoclaw-accent outline-none transition-all">
              </div>
              <div class="space-y-2">
                <label class="text-[10px] font-medium uppercase tracking-wide text-makoclaw-text-secondary/60 ml-1">Entropy (Temp)</label>
                <input v-model.number="orchestratorConfig.temperature" type="number" step="0.1" min="0" max="2" class="w-full bg-makoclaw-bg/20 border border-makoclaw-border/30 rounded-xl px-4 py-2.5 text-sm font-medium text-makoclaw-text focus:border-makoclaw-accent outline-none transition-all">
              </div>
              <div class="space-y-2">
                <label class="text-[10px] font-medium uppercase tracking-wide text-makoclaw-text-secondary/60 ml-1">Recursion Depth</label>
                <input v-model.number="orchestratorConfig.max_delegation_retries" type="number" class="w-full bg-makoclaw-bg/20 border border-makoclaw-border/30 rounded-xl px-4 py-2.5 text-sm font-medium text-makoclaw-text focus:border-makoclaw-accent outline-none transition-all">
              </div>
              <div class="flex items-center pt-6 ml-2">
                <label class="flex items-center gap-3 cursor-pointer group">
                  ...
                  <span class="text-[10px] font-medium uppercase tracking-tight text-makoclaw-text-secondary/70 group-hover:text-makoclaw-accent transition-colors">Safety Fallback</span>
                </label>
              </div>
            </div>
          </div>
    ...
    <div class="glass-panel rounded-2xl p-5 md:p-6 border border-makoclaw-border/50 relative overflow-hidden">
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            <h3 class="text-[11px] font-medium uppercase tracking-wide text-makoclaw-text-secondary/70">Specialist Fleet</h3>
             <p class="text-xs font-medium text-blue-400 mt-0.5">Active Neural Nodes: {{ specialists.length }}</p>
          </div>
        </div>
        <button
          @click="openCreateSpecialist"
          class="w-full sm:w-auto px-5 py-2.5 bg-white text-makoclaw-bg rounded-2xl text-[11px] font-semibold flex items-center justify-center gap-2 hover:bg-white/90 transition-all active:scale-95 shadow-xl shadow-white/5"
        >
      ...
      <!-- Empty States -->
      <div v-if="!orchestratorConfig.enabled" class="py-16 bg-makoclaw-bg/20 rounded-2xl text-center border border-dashed border-makoclaw-border/30 animate-pulse">
        <IconBlock class="w-16 h-16 mx-auto text-makoclaw-text-secondary/10 mb-6" />
        <p class="text-xs font-medium tracking-wide text-makoclaw-text-secondary/30">Matrix Disconnected</p>
        <p class="text-[10px] font-medium text-makoclaw-text-secondary/20 mt-2">Activate Orchestrator to interface with specialists</p>
      </div>

      <div v-else-if="specialists.length === 0" class="py-16 bg-makoclaw-bg/20 rounded-2xl text-center border border-dashed border-makoclaw-border/30">
        ...
        <p class="text-xs font-medium tracking-wide text-makoclaw-text-secondary/40">Hangar Empty</p>
            ...
            <div class="flex items-center gap-4">
              ...
              <div class="min-w-0">
                ...
                <div class="flex items-center gap-2 mt-1">
                   <span class="text-[9px] font-medium tracking-wide text-makoclaw-text-secondary opacity-50">{{ specialist.provider }}</span>
                   <span class="w-1 h-1 rounded-full bg-makoclaw-border"></span>
                   <span class="text-[9px] font-medium tracking-wide text-makoclaw-accent">{{ specialist.model.split('/').pop() }}</span>
                </div>
              </div>
            </div>
    ...
    <div class="glass-panel rounded-2xl p-5 md:p-6 border border-makoclaw-border/50 relative overflow-hidden group">
      <div class="flex items-center gap-4 mb-6">
        ...
        <div>
          <h3 class="text-[11px] font-medium tracking-wide text-makoclaw-text-secondary/70">Default Protocols</h3>
          <p class="text-xs font-medium text-makoclaw-text-secondary mt-0.5">Automated Mission Parameters</p>
        </div>
      </div>
      ...
      <div class="space-y-10">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div class="space-y-2">
            <label class="text-[10px] font-medium uppercase tracking-wide text-makoclaw-text-secondary/60 ml-1">Base Provider</label>
          ...
          <div class="space-y-2">
            <label class="text-[10px] font-medium uppercase tracking-wide text-makoclaw-text-secondary/60 ml-1">Standard Core</label>
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 gap-5 p-4 bg-makoclaw-bg/20 rounded-2xl border border-makoclaw-border/30">
          <div class="space-y-2">
            <label class="text-[9px] font-medium tracking-wide text-makoclaw-text-secondary/40 ml-1">Fluidity (Entropy)</label>
          ...
          <div class="space-y-2">
            <label class="text-[9px] font-medium tracking-wide text-makoclaw-text-secondary/40 ml-1">Output Cap</label>
          ...
          <div class="space-y-2">
            <label class="text-[9px] font-medium tracking-wide text-makoclaw-text-secondary/40 ml-1">Tool Endurance</label>
    ...
    <div class="flex justify-center md:justify-end pt-6">
      <button @click="saveSettings" :disabled="saving" class="w-full md:w-auto px-5 py-2.5 bg-makoclaw-accent hover:bg-makoclaw-accent-hover text-white rounded-xl text-[11px] font-semibold shadow-lg shadow-makoclaw-accent/30 transition-all flex items-center justify-center gap-2 disabled:opacity-50 active:scale-95 group">
