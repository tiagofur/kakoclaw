    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
      <div class="glass-panel rounded-2xl p-5 border border-makoclaw-border/50 transition-all duration-300 hover:border-makoclaw-accent/30 hover:shadow-lg">
        <div class="flex items-center gap-3 mb-5">
          ...
          <h3 class="text-xs font-medium tracking-wide text-makoclaw-text-secondary/70 uppercase">Web Infrastructure</h3>
        </div>
        <div class="space-y-4">
      ...
      <div class="glass-panel rounded-2xl p-5 border border-makoclaw-border/50 transition-all duration-300 hover:border-makoclaw-accent/30 hover:shadow-lg">
        <div class="flex items-center gap-3 mb-5">
          ...
          <h3 class="text-xs font-medium tracking-wide text-makoclaw-text-secondary/70 uppercase">Gateway Layer</h3>
        </div>
        <div class="space-y-4">
    ...
    <div class="glass-panel rounded-2xl p-5 border border-makoclaw-border/50 transition-all duration-300 hover:shadow-lg">
      <div class="flex items-center justify-between mb-5">
        <div class="flex items-center gap-3">
          ...
          <h3 class="text-xs font-medium tracking-wide text-makoclaw-text-secondary/70 uppercase">Utility Parameters</h3>
        </div>
        <button
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-5" v-if="configData?.tools?.web?.search">
        <div class="space-y-2">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div class="space-y-6">
          ...
          <div class="p-6 bg-makoclaw-bg/40 rounded-3xl border border-makoclaw-border/50 space-y-3 shadow-inner">
            <label v-for="(val, opt) in exportOptions" :key="opt" class="flex items-center justify-between p-3 rounded-xl hover:bg-white/5 cursor-pointer transition-colors group">
              ...
              <input type="checkbox" v-model="exportOptions[opt]" class="w-5 h-5 rounded-lg border-2 border-makoclaw-border bg-makoclaw-bg text-makoclaw-accent focus:ring-makoclaw-accent transition-colors">
            </label>
          </div>
          <button
            @click="exportBackup"
          ...
          >
            <ArrowDownTrayIcon v-if="!exporting" class="w-5 h-5 mr-3 transition-transform group-hover:-translate-y-1" />
            <span v-else class="w-5 h-5 border-4 border-white/20 border-t-white rounded-full animate-spin mr-3"></span>
        ...
        <div class="space-y-6">
          ...
          <div class="border-2 border-dashed border-makoclaw-border/50 rounded-3xl p-6 hover:border-makoclaw-accent/50 transition-all group relative bg-makoclaw-bg/20">
            ...
            <div v-if="!selectedFile" @click="$refs.fileInput.click()" class="flex flex-col items-center justify-center py-4 cursor-pointer">
              ...
              <p class="text-xs font-medium mt-3 text-makoclaw-text-secondary/50 group-hover:text-makoclaw-text-secondary">Load .makoclaw Module</p>
            </div>
            <div v-else class="space-y-4">
              <div class="flex items-center gap-4 p-4 bg-makoclaw-bg/60 rounded-2xl border border-makoclaw-border/50 shadow-inner">
                <div class="p-2 bg-makoclaw-accent/10 rounded-xl text-makoclaw-accent"><DocumentIcon class="w-6 h-6"/></div>
                ...
                <button @click="clearSelectedFile" class="p-2 text-makoclaw-text-secondary hover:text-red-400 transition-colors"><XMarkIcon class="w-4 h-4"/></button>
              </div>
              <button
                @click="importBackup"
              ...
              >
                <BoltIcon v-if="!importing" class="w-5 h-5 mr-3 transition-transform group-hover:scale-110" />
                <span v-else class="w-5 h-5 border-4 border-white/20 border-t-white rounded-full animate-spin mr-3"></span>
