<template>
  <div class="space-y-5 max-w-6xl mx-auto animate-fade-in-up">
    ...
    <div class="glass-panel rounded-2xl p-5 md:p-6 border border-makoclaw-border/50 relative overflow-hidden group">
      <div class="absolute -top-12 -right-12 w-48 h-48 bg-makoclaw-accent/10 blur-[60px] rounded-full opacity-50 group-hover:opacity-100 transition-opacity" />
      ...
      <div class="relative z-10">
        <div class="flex flex-col sm:flex-row items-center justify-between gap-4 mb-5">
          ...
          <button
            class="w-full sm:w-auto px-6 py-3 bg-makoclaw-accent hover:bg-makoclaw-accent-hover text-white rounded-xl text-xs font-black uppercase tracking-widest shadow-xl shadow-makoclaw-accent/10 transition-all flex items-center justify-center gap-2 active:scale-95 disabled:opacity-30 group/save"
        ...
        >
          <div class="w-12 h-12 border-4 border-makoclaw-accent/20 border-t-makoclaw-accent rounded-full animate-spin" />
          <p class="text-[10px] font-medium tracking-wide text-makoclaw-text-secondary/50">
        ...
        >
          ...
          <div class="bg-blue-500/5 border border-blue-500/10 rounded-xl p-4 flex items-start gap-3">
            <div class="p-2.5 rounded-xl bg-blue-500/10 text-blue-400 mt-1">
              <InformationCircleIcon class="w-5 h-5" />
            </div>
            <div>
              <p class="text-[11px] font-medium tracking-wide text-makoclaw-text mb-2">
                Protocol Architecture
              </p>
              <ul class="space-y-2">
                <li class="flex items-center gap-2 text-xs font-medium text-makoclaw-text-secondary/70">
                  <span class="w-1 h-1 rounded-full bg-makoclaw-accent" />
            ...
            <table class="w-full text-left text-sm border-collapse">
              <thead>
                <tr class="bg-makoclaw-bg/40 border-b border-makoclaw-border/50">
                  ...
                  <th
                    scope="col"
                    class="px-5 py-3 text-[10px] font-medium uppercase tracking-wide text-makoclaw-text-secondary/50 hidden md:table-cell"
                  >
                    Function Definition
                  </th>
                </tr>
              ...
              <tbody class="divide-y divide-makoclaw-border/30 text-makoclaw-text">
                ...
                <tr class="bg-makoclaw-accent/[0.03] group/row">
                  <td class="px-5 py-4 font-semibold text-makoclaw-accent">
                    <span class="flex items-center gap-3">
                      <span class="w-2 h-2 rounded-full bg-makoclaw-accent animate-pulse" />
                      Master Override (*)
                    </span>
                  </td>
                  <td class="px-6 py-4 text-center">
                    <label class="inline-flex items-center cursor-pointer">
                      <input
                        type="checkbox"
                        :checked="rolePermissions.admin?.includes('*')"
                        class="sr-only peer"
                        @change="toggleWildcard('admin', $event.target.checked)"
                      >
                      <div class="w-10 h-5 bg-makoclaw-bg/80 rounded-full peer peer-checked:bg-makoclaw-accent after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:after:translate-x-5 relative transition-all" />
                    </label>
                  </td>
                  <td class="px-6 py-4 text-center">
                    <label class="inline-flex items-center cursor-pointer">
                      <input
                        type="checkbox"
                        :checked="rolePermissions.user?.includes('*')"
                        class="sr-only peer"
                  ...
                  <td class="px-5 py-4 text-[10px] font-medium text-makoclaw-text-secondary/40 uppercase tracking-wide hidden md:table-cell">
                    Total Neural Integration
                  </td>
                </tr>
                ...
                <tr class="bg-amber-500/[0.03] group/row">
                  <td class="px-5 py-4 font-semibold text-amber-500">
                    <span class="flex items-center gap-3">
                      <LockClosedIcon class="w-4 h-4 opacity-40" />
                      Exec Restricted
                    </span>
                  </td>
                  <td class="px-6 py-4 text-center">
                    <input
                      type="checkbox"
                      :checked="rolePermissions.admin?.includes('exec_restricted')"
                      :disabled="rolePermissions.admin?.includes('*')"
                      class="w-5 h-5 rounded-lg border-2 border-makoclaw-border bg-makoclaw-bg text-makoclaw-accent focus:ring-makoclaw-accent disabled:opacity-20 cursor-pointer"
                  ...
                  <td class="px-6 py-4 text-center">
                    <input
                      type="checkbox"
                      :checked="rolePermissions.user?.includes('exec_restricted')"
                      :disabled="rolePermissions.user?.includes('*')"
                      class="w-5 h-5 rounded-lg border-2 border-makoclaw-border bg-makoclaw-bg text-makoclaw-accent focus:ring-makoclaw-accent disabled:opacity-20 cursor-pointer"
                      @change="toggleTool('user', 'exec_restricted', $event.target.checked)"
                    >
                  </td>
                  <td class="px-5 py-4 text-[10px] font-medium text-makoclaw-text-secondary/40 uppercase tracking-wide hidden md:table-cell">
                    Sandboxed OS Interaction
                  </td>
                </tr>
                ...
                <tr
                  ...
                  :key="tool.name"
                  class="hover:bg-white/[0.02] transition-colors group/tool"
                >
                  <td class="px-8 py-5">
                    <div class="flex items-center gap-3">
                      <div
                        class="w-2 h-2 rounded-full transition-colors"
                        :class="isRestrictedTool(tool.name) ? 'bg-orange-400' : 'bg-makoclaw-text-secondary/20 group-hover/tool:bg-makoclaw-accent'"
                      />
                      <span class="font-bold tracking-tight text-makoclaw-text group-hover/tool:text-makoclaw-accent transition-colors">
                        {{ tool.name }}
                      </span>
                      <span
                        v-if="isRestrictedTool(tool.name)"
                        class="text-[8px] font-medium uppercase text-orange-400 bg-orange-400/10 px-1.5 py-0.5 rounded"
                      >
                        Audited
                      </span>
                    </div>
                  ...
                  <td class="px-6 py-5 text-center">
                    <input
                      ...
                      :disabled="rolePermissions.admin?.includes('*')"
                      class="w-5 h-5 rounded-lg border-2 border-makoclaw-border bg-makoclaw-bg text-makoclaw-accent focus:ring-makoclaw-accent disabled:opacity-20 cursor-pointer transition-all"
                  ...
                  <td class="px-8 py-5 text-[11px] font-medium text-makoclaw-text-secondary/60 italic hidden md:table-cell group-hover/tool:text-makoclaw-text-secondary transition-colors">
                    {{ tool.description }}
                  </td>
                </tr>
              </tbody>
          ...
          <div class="glass-panel p-5 rounded-2xl bg-makoclaw-surface/30 border border-makoclaw-border/50">
            <div class="flex items-center gap-3 mb-5">
              <CommandLineIcon class="w-5 h-5 text-makoclaw-text-secondary/50" />
              <h4 class="text-[10px] font-bold uppercase tracking-widest text-makoclaw-text-secondary/60">
                Restricted Paradigm Set
              </h4>
            </div>
            <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3">
              <div
                ...
                :key="cmd"
                class="px-4 py-2.5 bg-makoclaw-bg/60 border border-makoclaw-border/50 rounded-xl text-[10px] font-black font-mono text-makoclaw-text-secondary/80 text-center hover:border-makoclaw-accent/30 hover:text-makoclaw-accent transition-all cursor-default"
              >
                {{ cmd }}
              </div>
            </div>
...
const toggleWildcard = (r, c) => {
  rolePermissions.value[r] = c ? ['*'] : []
}
const toggleTool = (r, t, c) => {
  if (!rolePermissions.value[r]) rolePermissions.value[r] = []
  const w = rolePermissions.value[r].indexOf('*')
  if (w !== -1) rolePermissions.value[r].splice(w, 1)
...
const loadPermissions = async () => {
  ...
  try {
    ...
    toast.error('Failed to sync permissions: ' + (err.response?.data?.error || err.message))
  } finally {
    loading.value = false
  }
}
...
const savePermissions = async () => {
  ...
  try {
    ...
    toast.error('Failed to commit permissions: ' + (err.response?.data?.error || err.message))
  } finally {
    saving.value = false
  }
}
