  ...
  <div class="space-y-6 animate-fade-in-up pb-10 max-w-5xl mx-auto">
    ...
    <div class="glass-panel rounded-3xl p-6 border border-makoclaw-border/50 transition-all hover:border-makoclaw-accent/30 relative overflow-hidden group">
      ...
      <div class="overflow-x-auto custom-scrollbar relative z-10 bg-makoclaw-bg/20 rounded-2xl border border-makoclaw-border/30">
        <table class="w-full text-left text-sm whitespace-nowrap">
          <thead class="bg-makoclaw-surface/50 border-b border-makoclaw-border/50 text-[10px] text-makoclaw-text-secondary/80 font-black uppercase tracking-widest">
            <tr>
              ...
              <th class="px-6 py-4">Link Status</th>
          ...
          <tbody class="divide-y divide-makoclaw-border/30 text-makoclaw-text">
            <tr v-for="u in usersList" :key="u.id" class="hover:bg-makoclaw-accent/5 transition-colors group">
              <td class="px-6 py-4 font-mono text-[10px] text-makoclaw-text-secondary/50">{{ u.id }}</td>
              <td class="px-6 py-4">
                <span class="font-bold text-sm tracking-tight group-hover:text-makoclaw-accent transition-colors">{{ u.username }}</span>
              </td>
              <td class="px-6 py-4">
                <span class="px-3 py-1 text-[9px] font-black tracking-widest uppercase rounded-lg shadow-sm"
              ...
              <td class="px-6 py-4">
                <span v-if="u.blocked" class="px-3 py-1 text-[9px] font-black tracking-widest uppercase rounded-lg bg-red-500/10 text-red-500 border border-red-500/20 flex items-center gap-1.5 w-max">
                  <NoSymbolIcon class="w-3 h-3" /> Offline
                </span>
                <span v-else class="flex items-center gap-1.5 px-3 py-1 text-[9px] font-black tracking-widest uppercase rounded-lg bg-makoclaw-success/10 text-makoclaw-success border border-makoclaw-success/20 w-max shadow-[0_0_10px_rgba(16,185,129,0.1)]">
                  <span class="w-1.5 h-1.5 rounded-full bg-makoclaw-success animate-pulse shadow-[0_0_5px_rgba(16,185,129,0.8)]"></span>
                  Active
                </span>
              </td>
              ...
              <td class="px-6 py-4 text-right">
                <div class="flex items-center justify-end gap-2">
                  <button @click="$emit('edit-user', u)" class="p-2 rounded-xl bg-makoclaw-surface border border-makoclaw-border/50 text-makoclaw-text-secondary hover:text-makoclaw-accent hover:border-makoclaw-accent/30 transition-all active:scale-95 shadow-sm" title="Modify Identity">
                    <PencilSquareIcon class="w-4 h-4"/>
                  </button>
                  <button v-if="!u.blocked" @click="$emit('block-user', u)" :disabled="authStore.user?.username === u.username" class="p-2 rounded-xl bg-makoclaw-surface border border-makoclaw-border/50 text-makoclaw-text-secondary hover:text-orange-400 hover:border-orange-400/30 transition-all active:scale-95 disabled:opacity-30 shadow-sm" title="Sever Protocol">
                    <ShieldExclamationIcon class="w-4 h-4"/>
                  </button>
                  <button v-else @click="$emit('unblock-user', u)" class="p-2 rounded-xl bg-makoclaw-surface border border-makoclaw-border/50 text-makoclaw-text-secondary hover:text-makoclaw-success hover:border-makoclaw-success/30 transition-all active:scale-95 shadow-sm" title="Restore Integration">
                    <CheckCircleIcon class="w-4 h-4"/>
                  </button>
                  <button @click="$emit('delete-user', u)" :disabled="authStore.user?.username === u.username" class="p-2 rounded-xl bg-makoclaw-surface border border-makoclaw-border/50 text-makoclaw-text-secondary hover:text-red-500 hover:border-red-500/30 transition-all active:scale-95 disabled:opacity-30 shadow-sm" title="Delete Node">
        ...
        <div v-if="!usersList || usersList.length === 0" class="p-8 text-center border-t border-makoclaw-border/30">
           <p class="text-xs font-bold uppercase tracking-widest text-makoclaw-text-secondary/50">No external nodes registered</p>
        </div>
      </div>
    </div>
  </div>
