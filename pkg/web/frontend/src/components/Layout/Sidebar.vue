<template>
  ...
  >
    ...
    <div class="h-16 flex items-center justify-between px-4 border-b border-kakoclaw-border">
      <div v-if="!uiStore.sidebarCollapsed" class="font-bold text-xl bg-gradient-to-r from-kakoclaw-accent to-purple-500 bg-clip-text text-transparent truncate flex items-center gap-2">
        <span>🐸</span> KakoClaw
      </div>
      <button
    ...
    <div class="flex-1 overflow-y-auto">
      ...
      <nav class="p-3 space-y-1">
        <router-link
        to="/dashboard"
        class="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-kakoclaw-bg transition-colors group relative"
        active-class="bg-kakoclaw-accent/10 text-kakoclaw-accent hover:bg-kakoclaw-accent/20"
      >
      ...
      <nav class="p-3 pt-0 space-y-1">
        <div v-if="!uiStore.sidebarCollapsed" class="px-3 py-1 text-[10px] font-semibold uppercase tracking-wider text-kakoclaw-text-secondary/60">Tools</div>
