...
func Run(opts Options) (*Result, error) {
	...
	makoclawHome, err := resolveMakoClawHome(opts.MakoClawHome)
	if err != nil {
		return nil, err
	}
	...
	actions, warnings, err := Plan(opts, openclawHome, makoclawHome)
	if err != nil {
		return nil, err
	}

	fmt.Println("Migrating from OpenClaw to makoclaw")
	fmt.Printf("  Source:      %s\n", openclawHome)
	...
	result := Execute(actions, openclawHome, makoclawHome)
	result.Warnings = warnings
	return result, nil
}

func Plan(opts Options, openclawHome, makoclawHome string) ([]Action, []string, error) {
	var actions []Action
	var warnings []string
	...
	if !opts.WorkspaceOnly {
		...
		if err != nil {
			...
			actions = append(actions, Action{
				...
				Source:      configPath,
				Destination: filepath.Join(makoclawHome, "config.json"),
				Description: "convert OpenClaw config to makoclaw format",
			})
	...
	if !opts.ConfigOnly {
		...
		dstWorkspace := resolveWorkspace(makoclawHome)
...
func Execute(actions []Action, openclawHome, makoclawHome string) *Result {
	result := &Result{}

	for _, action := range actions {
		switch action.Type {
		case ActionConvertConfig:
			if err := executeConfigMigration(action.Source, action.Destination, makoclawHome); err != nil {
...
func executeConfigMigration(srcConfigPath, dstConfigPath, makoclawHome string) error {
	...
	if _, err := os.Stat(dstConfigPath); err == nil {
		...
		if err != nil {
			return fmt.Errorf("loading existing makoclaw config: %w", err)
		}
...
func resolveMakoClawHome(override string) (string, error) {
	if override != "" {
		return expandHome(override), nil
	}
	if envHome := os.Getenv("MAKOCLAW_HOME"); envHome != "" {
		return expandHome(envHome), nil
	}
	home, err := os.UserHomeDir()
	if err != nil {
