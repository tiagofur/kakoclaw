...
func Run(opts Options) (*Result, error) {
	...
	kakoclawHome, err := resolveKakoclawHome(opts.KakoclawHome)
	if err != nil {
		return nil, err
	}
	...
	actions, warnings, err := Plan(opts, openclawHome, kakoclawHome)
	if err != nil {
		return nil, err
	}

	fmt.Println("Migrating from OpenClaw to KakoClaw")
	fmt.Printf("  Source:      %s\n", openclawHome)
	...
	result := Execute(actions, openclawHome, kakoclawHome)
	result.Warnings = warnings
	return result, nil
}

func Plan(opts Options, openclawHome, kakoclawHome string) ([]Action, []string, error) {
	var actions []Action
	var warnings []string
	...
	if !opts.WorkspaceOnly {
		...
		if err != nil {
			...
			actions = append(actions, Action{
				...
				Source:      configPath,
				Destination: filepath.Join(kakoclawHome, "config.json"),
				Description: "convert OpenClaw config to KakoClaw format",
			})
	...
	if !opts.ConfigOnly {
		...
		dstWorkspace := resolveWorkspace(kakoclawHome)
...
func Execute(actions []Action, openclawHome, kakoclawHome string) *Result {
	result := &Result{}

	for _, action := range actions {
		switch action.Type {
		case ActionConvertConfig:
			if err := executeConfigMigration(action.Source, action.Destination, kakoclawHome); err != nil {
...
func executeConfigMigration(srcConfigPath, dstConfigPath, kakoclawHome string) error {
	...
	if _, err := os.Stat(dstConfigPath); err == nil {
		...
		if err != nil {
			return fmt.Errorf("loading existing KakoClaw config: %w", err)
		}
...
func resolveKakoclawHome(override string) (string, error) {
	if override != "" {
		return expandHome(override), nil
	}
	if envHome := os.Getenv("KakoClaw_HOME"); envHome != "" {
		return expandHome(envHome), nil
	}
	home, err := os.UserHomeDir()
	if err != nil {
	...
	return filepath.Join(home, ".KakoClaw"), nil
}
