...
- `cmd/KakoClaw/main.go` is a manual command dispatcher (no Cobra): `agent`, `gateway`, `web`, `cron`, `skills`, `auth`, `doctor`, `migrate`, `onboard`.
...
- Default persistent paths are user-home based (e.g., config `~/.KakoClaw/config.json`, workspace `~/.KakoClaw/workspace`), and workspace path comes from `cfg.WorkspacePath()`.
...
- Skills loading precedence is strict: workspace skills override global (`~/.KakoClaw/skills`), which override built-in repo skills (`skills/`).
