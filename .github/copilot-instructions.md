...
- `cmd/MakoClaw/main.go` is a manual command dispatcher (no Cobra): `agent`, `gateway`, `web`, `cron`, `skills`, `auth`, `doctor`, `migrate`, `onboard`.
...
- Default persistent paths are user-home based (e.g., config `~/.MakoClaw/config.json`, workspace `~/.MakoClaw/workspace`), and workspace path comes from `cfg.WorkspacePath()`.
...
- Skills loading precedence is strict: workspace skills override global (`~/.MakoClaw/skills`), which override built-in repo skills (`skills/`).
